...
consF f (UnsafeSingQueueF d h t) = UnsafeSingQueueF (succ d) (SomeF f : h) t

checkF :: SingQueueF f q -> SingQueueF f q
checkF (UnsafeSingQueueF d [] t) = UnsafeSingQueueF d (reverse t) []
checkF q                         = q

unconsF :: SingQueueF f (Tr st st' <| q) -> (f st st, SingQueueF f q)
unconsF (UnsafeSingQueueF d h t) =
    case h of
      SomeF f : h' -> unsafeCoerce (f, checkF $ UnsafeSingQueueF (pred d) h' t)
      []           -> error "impossible happened!"

snocF :: SingQueueF f q
      -> f st st'
      -> SingQueueF f (q |> Tr st st')
snocF (UnsafeSingQueueF d h t) f = checkF (UnsafeSingQueueF (succ d) h (SomeF f : t))
...
toIsQueueF (UnsafeSingQueueF _ [] []) = unsafeCoerce IsEmptyF
...
queueFDepth (UnsafeSingQueueF d _ _) = d
